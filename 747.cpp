...
int main() {
	...
	for (int i = 0; i != T; ++i) {
		...
		cin >> K;
		int number;
		vector<pair<mydata, index_num>> initial_seq;
		for (int i = 0; i != N; ++i) {
			cin >> number;
			initial_seq.push_back(make_pair(number, i + 1));
		}
		vector<pair<mydata, index_num>> current_seq(initial_seq);
		while (!current_seq.empty()) {
			//Step1
			vector<pair<mydata, index_num>> temp_seq;
			if (current_seq.size() >= K) {
				temp_seq.insert(temp_seq.end(), current_seq.begin(), current_seq.begin() + K);
				current_seq.erase(current_seq.begin(), current_seq.begin() + K);
			}
			else {
				temp_seq.insert(temp_seq.end(), current_seq.begin(), current_seq.end());
				current_seq.clear();
			}
			vector<pair<mydata, index_num>>::iterator vmax = temp_seq.begin();
			for (vector<pair<mydata, index_num>>::iterator iter = temp_seq.begin(); iter != temp_seq.end(); ++iter) {
				if (iter->first > vmax->first)
					vmax = iter;
			}
			for (vector<pair<mydata, index_num>>::iterator iter = temp_seq.begin(); iter != vmax; ++iter) {
				iter->first -= 1;
			}
			for (vector<pair<mydata, index_num>>::iterator iter = vmax + 1; iter != temp_seq.end(); ++iter) {
				iter->first -= 1;
			}
			//Step2
			cout << vmax->second<<" ";
